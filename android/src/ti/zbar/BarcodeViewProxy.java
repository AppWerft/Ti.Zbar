...
public class BarcodeViewProxy extends TiViewProxy {
	 ...
	 * 
	 * @see
	 * org.appcelerator.titanium.proxy.TiViewProxy#createView(android.app.Activity
	 * )
	 */
	@Override
	...
	public void handleCreationDict(KrollDict options) {
		...
		if (hasProperty(BarcodeInputArgs.CONTINUOUS)) {
			args.setContinuous(TiConvert
					.toBoolean(getProperty(BarcodeInputArgs.CONTINUOUS)));
		}
		if (hasProperty(BarcodeInputArgs.USE_JIS_ENCODING)) {
			args.setUseJISEncoding(TiConvert
					.toBoolean(getProperty(BarcodeInputArgs.USE_JIS_ENCODING)));
		}
		if (hasProperty(BarcodeInputArgs.SCAN_FROM_IMAGE_CAPTURE)) {
			args.setScanFromImageCapture(TiConvert
					.toBoolean(getProperty(BarcodeInputArgs.SCAN_FROM_IMAGE_CAPTURE)));
		}
		if (hasProperty(BarcodeInputArgs.USE_FRONT_CAMERA)) {
			if (TiConvert
					.toBoolean(getProperty(BarcodeInputArgs.USE_FRONT_CAMERA))) {
		...
		if (hasProperty(BarcodeInputArgs.OVERLAY)) {
			...
			if (overlay.containsKey(BarcodeInputArgs.OVERLAY_COLOR)) {
				args.setColor((String) overlay
						.get(BarcodeInputArgs.OVERLAY_COLOR));
			}
			if (overlay.containsKey(BarcodeInputArgs.OVERLAY_LAYOUT)) {
				args.setLayout((String) overlay
						.get(BarcodeInputArgs.OVERLAY_LAYOUT));
			}
			if (overlay.containsKey(BarcodeInputArgs.OVERLAY_IMAGE_NAME)) {
				args.setImageName((String) overlay
						.get(BarcodeInputArgs.OVERLAY_IMAGE_NAME));
			}
			if (overlay.containsKey(BarcodeInputArgs.OVERLAY_ALPHA)) {
				args.setAlpha(TiConvert.toFloat(overlay
						.get(BarcodeInputArgs.OVERLAY_ALPHA)));
			}
		}
	}
